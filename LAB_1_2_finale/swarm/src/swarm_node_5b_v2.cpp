...
int main (int argc, char** argv){
    ...
    //ROS Initialization
    ros::init(argc, argv, "swarm_node_5b_v2");
...
// ***    
    ...
    ros::Subscriber subToGoal = 
        nh_glob.subscribe<geometry_msgs::Point>("leader",1,virtualLeaderCallback);

    // What the node publishes to.
    //ros::Publisher pubPos = nh_glob.advertise<NameAndPose::node_name>("/pos",1);
    ros::Publisher pubPos = nh_loc.advertise<swarm::NameAndPose>("new_pos",1);
...
// ***
    ...
    swarm::NameAndPose new_pos;
    new_pos.node_name = ros::this_node::getName();
    ...
    while (ros::ok()){
        ...
        new_pos.node_pose = pos;
        pubPos.publish(new_pos) ;
